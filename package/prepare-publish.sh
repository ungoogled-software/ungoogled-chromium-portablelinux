...
./utilities/submit_github_binary.py --skip-checks --tag ${TAG} --username clickot --output config/platforms/linux_portable/64bit/ ${PATH_TO_BUILD_REPO}/ungoogled-chromium_${TAG}*.tar.xz 
./utilities/submit_github_binary.py --skip-checks --tag ${TAG} --username clickot --output config/platforms/appimage/64bit/ ${PATH_TO_BUILD_REPO}/ungoogled-chromium_${TAG}*.AppImage 
